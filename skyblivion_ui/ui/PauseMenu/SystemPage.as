...
class SystemPage extends MovieClip
{
	...
	var bDefaultButtonVisible: Boolean; //CrEaToXx: new in SkyUI 5.2
	var bIsRemoteDevice: Boolean; //CrEaToXx: new in SkyUI 5.2
	...
	private var _characterSelectionControls: Object; //CrEaToXx: new in SkyUI 5.2
	...
	private var _showModMenu: Boolean; //CrEaToXx: new in SkyUI 5.2
	private var _deleteButton: Object; //CrEaToXx: new in SkyUI 5.2
	...
	function set currentState(aiNewState: Number): Void
	{
		...
		//GameDelegate.call("SetJournalMenuState", [aiNewState]);//VR specific?
		currentState();// how it's done in SkyUI 5.2
	}

	function OnSaveDataEventSaveSUCCESS()
	{
		if(iPlatform == SystemPage.CONTROLLER_ORBIS)// SkyUI 5.2 way of checking for ORBIS
		{
			bMenuClosing = true;
			EndState();
		}
	...
	public function handleInput(details: InputDetails, pathToFocus: Array): Boolean
	{
		...
		if (bRemapMode || bMenuClosing || bSavingSettings || iCurrentState == SystemPage.TRANSITIONING) {
			...
			if ((details.navEquivalent == NavigationCode.GAMEPAD_L2 || details.navEquivalent == NavigationCode.GAMEPAD_R2) && isConfirming()) {
				bhandledInput = true;
			} else if ((details.navEquivalent == NavigationCode.GAMEPAD_X || details.code == 88) && iCurrentState == SystemPage.SAVE_LOAD_STATE) {
			
				if(iPlatform == SystemPage.CONTROLLER_ORBIS)
				{
					GameDelegate.call("ORBISDeleteSave",[]);
				} else {
					ConfirmDeleteSave();
				}
				bhandledInput = true;
			} else if ((details.navEquivalent == NavigationCode.GAMEPAD_Y || details.code == 84) && iCurrentState == SystemPage.SAVE_LOAD_STATE && !SaveLoadListHolder.isSaving) {
	...
	function onSettingsCategoryPress(): Void
	{
		...
		switch (SettingsList.selectedIndex) {
			...
			case 1:
				...
				{text: "$DDOF Intensity", movieType: 0}]; //CrEaToXx: fixed missing DOF setting which crashed the game onPress...wasn't present in Legendary!
	...
	function SetPlatform(a_platform: Number, a_bPS3Switch: Boolean): Void //CrEaToXx: restored original way of setting the platform
	{
		...
		if (a_platform != 0) {
			...
			_characterSelectionControls = {keyCode:279};// 360_Y, was missing in VR
			...
			_characterSelectionControls = {keyCode:20}; // T, was missing in VR
		}
	...
	{
		bShowKinectTunerButton = abFlag == true; //remove kinect toggle from options?
	}
